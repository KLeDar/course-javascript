...
export default class SimpleChat {
  constructor() {
    this.wsClient = new WSClient(
      `ws://${location.host}/simple-chat/ws`,
  ...
  onUpload(data) {
    ...
    fetch('/simple-chat/upload-photo', {
      method: 'post',
      body: JSON.stringify({
  ...
  async onLogin(name) {
    ...
    this.ui.userPhoto.set(`/simple-chat/photos/${name}.png?t=${Date.now()}`);
  }
  ...
  onMessage({ type, from, data }) {
    ...
    if (type === 'hello') {
      ...
      for (const avatar of avatars) {
        avatar.style.backgroundImage = `(/simple-chat/photos/${
          data.name
        }.png?t=${Date.now()})`;
      }
    }
